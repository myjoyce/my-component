...
var utils=(function(){
    ...
    if(typeof tagName === 'string'){
        for(var k=0;k<ary.length;k++){//数组会减小的情况，必须使用这样的方式判断长度，不能和上面一样
...
//一直找到哥哥元素节点为止，如果没有哥哥元素节点，返回null即可
...
//insertBefore:把新元素(newEle)添加到指定元素(oldEle)前面
...
function getElementsByClass(strClass,context){
    ...
    if(flag){
        return this.listToArray(context.getElementsByClass(strClass));
    }
    var ary = [],
        strClassAry = strClass.replace(/(^+|+$)/g,"").split(/+/g);
    var nodeList = context.getElementsByName("*");
    for(var i = 0,len = nodeList.length;i<len;i++){
        var curNode = nodeList[i];
        var isOk = true;
